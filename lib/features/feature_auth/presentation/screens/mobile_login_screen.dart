...
import 'package:homdic/features/feature_auth/presentation/widgets/cutom_clippath_signup.dart';
...
class _MobileLoginScreenState extends State<MobileLoginScreen> {
  ...
  initSmsRetriever() async {
    await AndroidSmsRetriever.listenForSms().then((value) {
      ...
      BlocProvider.of<LoginBloc>(context)
          .add(LoadCodeCheck(codeController.text));
    });
  }
                      ...
                      children: [
                        ...
                        BlocConsumer<LoginBloc, LoginState>(
                          listenWhen: (previous, current) {
                            if (previous.loginDataStatus ==
                                current.loginDataStatus) {
                              return false;
                            }
                            return true;
                          },
                          ...
                          listener: (context, state) {
                            if (state.loginDataStatus is LoginDataError) {
                              LoginDataError loginDataError =
                                  state.loginDataStatus as LoginDataError;
                              showSnack(context, loginDataError.errorMessage,
                                  Colors.red);
                            }

                            if (state.loginDataStatus is LoginDataCompleted) {
                              LoginDataCompleted loginDataCompleted =
                                  state.loginDataStatus as LoginDataCompleted;
                              showSnack(
                                  context, "کد ورود ارسال شد", Colors.green);
                                ...
                                onPressed: () {
                                  if (_formKey.currentState!.validate()) {
                                    BlocProvider.of<LoginBloc>(context).add(
                                        LoadLoginSms(phoneController.text));
                                  }
                                },
                                child: const Text(
                                  'ارسال کد ورود',
  ...
  Future<void> saveDataToPrefs(CodeModel codeModel) async {
    ...
    await prefs.setString('user_mobile',
        codeModel.mobile.toString().toEnglishDigit() ?? "شماره تماس");
  }
                    ...
                    listener: (context, state) {
                      if (state.codeCheckStatus is CodeCheckError) {
                        CodeCheckError codeCheckError =
                            state.codeCheckStatus as CodeCheckError;
                        showSnack(
                            context, codeCheckError.errorMessage, Colors.red);
                      }

                      if (state.codeCheckStatus is CodeCheckCompleted) {
                        CodeCheckCompleted codeCheckCompleted =
                            state.codeCheckStatus as CodeCheckCompleted;
                        if (codeCheckCompleted.codeModel.name != null) {
                          showSnack(
                              context,
                              "${codeCheckCompleted.codeModel.name} به نیاز خوش آمدید",
                              Colors.green);
                        }
